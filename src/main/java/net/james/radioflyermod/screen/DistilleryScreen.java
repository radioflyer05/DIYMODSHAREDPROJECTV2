...
import net.james.radioflyermod.screen.renderer.EnergyInfoArea;
import net.james.radioflyermod.screen.renderer.FluidTankRenderer;
import net.james.radioflyermod.util.MouseUtil;
...
public class DistilleryScreen extends AbstractContainerScreen<DistilleryMenu> {
    ...
    private void assignFluidRenderer() {
        renderer = new FluidTankRenderer(30000, true, 67, 16);
    }
    ...
    private void renderFluidAreaTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int x, int y) {
        if(isMouseAboveArea(pMouseX, pMouseY, x, y, 48, 56)) {
    ...
    protected void renderBg(PoseStack pPoseStack, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        renderer.render(pPoseStack, x + 48, y + 56, menu.getFluidStack());
    }

    private void renderProgressArrow(PoseStack pPoseStack, int x, int y) {
        if(menu.isCrafting()) {
            blit(pPoseStack, x + 8, y + 25, 178, 0, 10, menu.getScaledProgress());
        }
    }
